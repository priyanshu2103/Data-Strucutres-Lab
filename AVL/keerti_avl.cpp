...
{
	if(root->lheight-root->rheight==1||root->lheight-root->rheight==0||root->lheight-root->rheight==-1) return false;
	return true;
}
...
bool avl::insert(int x)
{
	...
	while(!imbalanced(q))
	{
		if(q->parent==NULL) return true;
		q=q->parent;
		if(q->left==NULL) q->lheight=0;
		else q->lheight=max(q->left->lheight,q->left->rheight)+1;
		if(q->right==NULL) q->rheight=0;
		else q->rheight=max(q->right->lheight,q->right->rheight)+1;
	}
...
int main()
{
	...
	int choice;
	bool result;
	/*cout<<"enter no to insert, -1 to exit\n";
	while(1)
	{
		cin>>i;
		if(i==-1) break;
	...
	cout<<"enter no to search, -1 to exit\n";
	while(1)
	{	
		cin>>i;
		if(i==-1) break;
		if(t->search(i)) cout<<"No is present\n";
	...
	// t->insert(24);
	// t->insert(7);
	node *r=t->root;//giveroot();
	inorder(r);
	cout<<endl;
	preorder(r);
	cout<<endl;
	postorder(r);
	//cout<<r->right->data<<" "<<r->right->left->data<<" "<<r->right->right->data<<" "<<r->right->right->right->data<<" "<<r->right->right->right->left->data<<endl;
	cout<<endl;
	return 0;*/
